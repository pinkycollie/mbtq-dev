...
## 2024-05-14 - Optimize Webhook Retries
**Learning:** Sequential awaits in webhook retries create an O(N) network bottleneck that can block background tasks.
**Action:** Replace sequential loop with chunked concurrent execution using Promise.all() to improve throughput while avoiding rate limits or memory exhaustion.
