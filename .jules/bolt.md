...
## 2026-04-26 - Optimize Webhook Retries Concurrency
**Learning:** Sequential awaits in asynchronous worker loops (like webhook retries) cause O(N) network blocking, making the execution time scale linearly with network latency rather than processing time.
**Action:** Replaced sequential awaits with chunked concurrent execution using Promise.all (chunk size 10) to optimize throughput while preventing connection exhaustion or rate limits on external APIs.
