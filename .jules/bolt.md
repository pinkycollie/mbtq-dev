...
## 2024-05-05 - Chunked Concurrent Webhook Execution
**Learning:** Sequential `await` operations in webhook retry loops cause O(N) network blocking.
**Action:** Replace sequential loops with chunked concurrent execution using `Promise.all()` (e.g., chunk size of 10) to optimize performance without overwhelming the event loop or external APIs. Explicitly type callback parameters to avoid implicit any errors due to Prisma type inference issues.
