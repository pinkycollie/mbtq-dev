...
## 2024-10-25 - O(n) React List Mutations due to Array Index Keys
**Learning:** The `ActionLog` component used the array index (`idx`) as the React `key` for items in the `eventLog` list. Because new events were prepended to the array (`[event, ...prev]`), every new event shifted the indices of all existing items. This caused React to destroy and recreate the DOM nodes for the entire list on every single update, changing an O(1) DOM insertion into an O(n) rendering bottleneck.
**Action:** Always generate a unique ID (like `crypto.randomUUID()` or a stable timestamp+actor string) for prepended or reordered lists. Never use array indices as keys unless the list is strictly append-only or static.