...
## 2026-04-22 - Chunked concurrent webhook retries
**Learning:** Sequential async operations in loops (like webhook retries) cause O(N) network blocking. However, fully concurrent execution with Promise.all on a large dataset can overwhelm the event loop or external APIs.
**Action:** Use chunked concurrent execution with Promise.all (e.g., chunk size 10) to balance speed and stability when processing lists of network requests.
