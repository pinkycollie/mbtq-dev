...
## 2024-05-24 - React List Rendering Bottleneck with StateEventBus
**Learning:** Using array indices (`idx`) as keys when prepending items to a list from a global event bus (`StateEventBus`) causes React to re-render every item in the list, creating an O(n) rendering bottleneck as the list grows. This is especially noticeable with high-frequency event streams like agent activity logs.
**Action:** Always include a stable, unique identifier (e.g., `crypto.randomUUID()`) in the event payload emitted by the bus and use it as the React `key` when rendering dynamically growing lists, particularly when items are prepended.
