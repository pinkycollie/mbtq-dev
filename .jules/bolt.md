...
## 2024-11-20 - [Stable IDs in Event Bus for O(1) Rendering]
**Learning:** Using an array index as the React key for dynamically prepended list items (such as those emitted by `StateEventBus` in `SignVisualSystem` causing prepended values in `ActionLog`) causes React to re-render the entire list (O(n) complexity) on every event emission.
**Action:** When working with the custom `StateEventBus` and `SignVisualSystem` (and similarly dynamic lists), attach a stable identifier (e.g., `crypto.randomUUID()`) to events at the emission stage (in the `StateEventBus`), and use this stable `id` as the list item `key`.
