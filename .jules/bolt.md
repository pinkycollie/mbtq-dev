...
## 2025-03-01 - Chunked Concurrent Execution for Webhook Retries
**Learning:** Sequential `await` in loops for network operations (like webhook retries) causes O(N) network blocking, significantly slowing down background jobs.
**Action:** Replace sequential `await` loops with chunked concurrent execution using `Promise.all()` (e.g., chunk size of 10) to improve throughput without overwhelming the event loop or external APIs. Explicitly type callback parameters as `any` if Prisma type inference fails.
