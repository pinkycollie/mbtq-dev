...
export const ActionLog: React.FC<ActionLogProps> = ({ events }) => {
  return (
    <div
      className="bg-gray-900 rounded-lg p-4 max-h-64 overflow-y-auto focus:outline-none focus-visible:ring-2 focus-visible:ring-purple-500"
      tabIndex={0}
      role="region"
      ...
      <div className="space-y-2">
        {events.length === 0 ? (
          <div className="text-gray-500 text-sm italic text-center py-4">
            No activity yet. Waiting for agent...
          </div>
        ) : (
          events.map((event, idx) => {
            ...
            return (
              <div key={idx} className="flex items-start gap-3 text-sm">
                <span className={`${semantic.color} w-2 h-2 rounded-full mt-1.5 flex-shrink-0`} aria-hidden="true" />
